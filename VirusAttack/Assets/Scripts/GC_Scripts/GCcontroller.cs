...
public class GCcontroller : MonoBehaviourPunCallbacks//, IDamageable
{
	PhotonView view;
	...
	public static GCcontroller Instance;
	...
	void Update()
	{
		...
		if (view.IsMine) // if local player we can move around and other stuff
		{
			...
			animator.SetBool("IsWalking", Input.GetKey(KeyCode.V));
			bool IsShooting = Input.GetKey(KeyCode.W);
