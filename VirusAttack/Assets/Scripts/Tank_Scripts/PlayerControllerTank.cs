...
public class PlayerControllerTank : MonoBehaviourPunCallbacks
{
	PhotonView view;
	...
	public const float ShieldHealth = 500f;
	private float currentHealth = maxHealth;
	private float currentShieldHealth = ShieldHealth;
	...
	void Start()
	{
		...
		playerShieldText.text = "+" + currentShieldHealth;
	...
	void Update()
	{
		...
		if (view.IsMine) // if local player we can move around and other stuff
		{
			...
			//bool IsAttacking = Input.GetKey(KeyCode.Y);
			//animator.SetBool("IsAttacking", Input.GetKey(KeyCode.Y));
	...
	public void RPC_TakeDamage(float damage)
	{
		...
		if (Input.GetKey(KeyCode.F) & currentShieldHealth > 0)
		{
	...
	void lookAround()
	{
		...
		verticalLookRotation = Mathf.Clamp(verticalLookRotation, -65f, 65f);
